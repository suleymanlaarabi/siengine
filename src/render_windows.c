...
void sirender_draw_windows(ecs_iter_t *it) {
    ...
    for (uint32_t i = 0; i < it->count; i++) {
        ...
        SDL_GPURenderPass *pass =
            SDL_BeginGPURenderPass(render->frame.cmd, &color_target, 1, &depth_target);
        SDL_BindGPUGraphicsPipeline(pass, render->cubes.pipeline);

        for (uint32_t view_i = 0; view_i < render->frame.view_count; view_i++) {
            SIRenderView *view = &render->frame.views[view_i];
